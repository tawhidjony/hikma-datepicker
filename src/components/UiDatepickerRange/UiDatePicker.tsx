...
interface UiDatePickersProps {
  ...
  register?: { onChange: (value: { start: string | null; end: string | null }) => void };
  fullWidth?: boolean;
  errors?: boolean;
  dateFormat?: string;
  placeholder?: string;
...
const UiDatePickerRange = forwardRef<HTMLInputElement, UiDatePickersProps>((props, ref) => {
  const {
    className,
    selectedStyle,
    todayDateStyle,
  ...
  const isDateSelected = (date: Date) => {
    ...
    const isInRange = selectedEndDate && date > selectedStartDate && date < selectedEndDate;
  ...
  const handleDateClick = (date: Date) => {
    ...
    if (onChange) {
      onChange({
        start: selectionMode === 'start' ? formatDate(date, dateFormat) : selectedStartDate ? formatDate(selectedStartDate, dateFormat) : null,
        end: selectionMode === 'end' ? formatDate(date, dateFormat) : selectedEndDate ? formatDate(selectedEndDate, dateFormat) : null,
      });
    }
    if (register) {
      register.onChange({
        ...
        <input
          ...
          value={
            selectedStartDate && selectedEndDate
              ? `${formatDate(selectedStartDate, dateFormat)} - ${formatDate(selectedEndDate, dateFormat)}`
              : selectedStartDate
                ? `${formatDate(selectedStartDate, dateFormat)} - Select end date`
                : ""
          }
